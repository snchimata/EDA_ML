...
server<-(function(input, output, session) {
  ...
  output$MissingPattern<-renderPlot({
    ...
    aggr(DF1, col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, labels=names(data), cex.axis=.7, gap=3, ylab=c("Histogram of missing data","Pattern"))
    #}
  })
  ...
  output$CorPlot<-renderPlot(if(input$Go1){
    ...
    M <- cor(na.omit(DF1[, sapply(DF, is.numeric)]))
  ...
  output$ScatterAllPairs<-renderPlot({
    ...
    pairs(DF1[, sapply(DF, is.numeric)], 
  ...
  output$ScatterSinglePair<-renderPlotly(if(input$PlotType=="Scatter"){
    DF1<-DataTypeConversion()
    
    ggplot(DF1, aes_string(x=input$Xaxis, y=input$Yaxis)) +
  ...
  output$BoxPlot<-renderPlotly(if(input$PlotType=="Box"){
    ...
    ggplot(DF1,aes_string(x=input$GrByBoxs,y=input$BoxParam,fill=input$GrByBoxs)) +geom_boxplot()+theme_bw()
  ...
  output$tabPlot<-renderPlot(if(input$Go1 & input$PlotType=="Tabular"){
    ...
    DF1<-DataTypeConversion()
    tableplot(DF1,select_string =isolate(input$SelectTabPlot),sortCol=isolate(input$SortTabPlot),from=as.numeric(isolate(input$FilterTabPlot[1])),to=as.numeric(isolate(input$FilterTabPlot[2])))
  ...
  output$MosaicPlot<-renderPlot(if(input$PlotType=="Mosaic"){
    ...
    DF1<-DataTypeConversion()
    x<-table(DF1[,input$Mosaic1st],DF[,input$Mosaic2nd])
